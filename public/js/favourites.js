...
function toggleEditModal() {
  ...
  if (
    !stationElement?.classList.contains("active") ||
    !stationElement?.classList.contains("fav")
  )
    return;

  document.getElementById("editStationNameInput").value = currentStation.name;
  document.getElementById("editStationFaviconInput").value =
...
function favouritesAction(action) {
  ...
  if (action === "remove") {
    ...
    if (confirm(`Remove ${currentStation.name} from favourites?`)) {
      const index = favourites.stations.findIndex(
    ...
    let stationElement = document.getElementById(
      currentStation.uuid + "_search"
    );
    if (!stationElement?.classList.contains("active")) return;
    if (stationElement.classList.contains("fav")) return;
...
function renderFavourites() {
  ...
  if (!favourites.stations || favourites.stations.length === 0) {
    ...
    emptyMessage.innerHTML =
      "Favourites are empty. <br>Select the upper right dropdown and search for stations. <br>Select a station from the results and add it to the favourites via the dropdown.";
  ...
  favourites.stations.forEach((station) => {
    ...
    newFav.setAttribute(
      "class",
      "list-group-item list-group-item-action d-flex justify-content-between align-items-start fav"
    );
    newFav.setAttribute(
      "onclick",
