...
function updateUI() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    // Mark active station
    if (
      item.id === currentStation.uuid ||
      item.id === currentStation.uuid + "_search"
    ) {
...
function updateDropdownStatus() {
  ...
  const isBookmarked = document
    .getElementById(currentStation.uuid)
    ?.classList.contains("fav");
  dropdownStation.classList.add(
    isBookmarked ? "bi-bookmark-check" : "bi-bookmark-x"
  );
...
function initSortable() {
  return new Sortable(document.getElementById("favourites"), {
    ...
    onEnd: function (/**Event*/ evt) {
      const moved = favourites.stations.splice(evt.oldIndex, 1)[0];
      favourites.stations.splice(evt.newIndex, 0, moved);
      fetch("/setfavs", {
        method: "POST",
        headers: {
