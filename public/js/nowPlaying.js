...
function handleTrackInfo() {
  const isActiveFav = document
    .getElementById(currentStation.uuid)
    ?.classList.contains("fav");

  if (
    (!radio.paused && isActiveFav) ||
    (chromeCastPlayerState === "PLAYING" && isActiveFav)
  ) {
...
function handleTrackUpdate(info) {
  if (info.error) {
    ...
    if (!skipFirstTrackUpdate) {
      document.getElementById("title_" + currentStation.uuid).innerHTML =
        info.title
          ? `<a class="text-decoration-none text-body-secondary" href="https://www.youtube.com/results?search_query=${encodeURIComponent(
              info.title
            )}" target="_blank" title="Search on YouTube" onclick="event.stopPropagation();">${
              info.title
            }</a>`
          : "";
      if (info.title) {
      ...
      document.title = info.title
        ? `${currentStation.name} - ${info.title}`
        : currentStation.name;
    } else {
      document.getElementById("title_" + currentStation.uuid).innerHTML = "";
      if ("mediaSession" in navigator && !chromeCastIsConnected) {
