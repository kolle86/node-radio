...
function setSearchBy(criteria, element) {
  ...
  document
    .querySelectorAll("#searchBy .dropdown-item i")
    .forEach((icon) => icon.classList.remove("bi-check-lg"));
...
function setOrderBy(criteria, element) {
  ...
  document
    .querySelectorAll("#orderBy .dropdown-item i")
    .forEach((icon) => icon.classList.remove("bi-check-lg"));
  element.querySelector("i").classList.add("bi-check-lg");
}
/**
...
async function searchStations(searchTerm, searchBy, orderBy) {
  const url = `/search?searchterm=${encodeURIComponent(
    searchTerm
  )}&searchBy=${encodeURIComponent(searchBy)}&orderBy=${encodeURIComponent(
    orderBy
  )}`;
...
function appendSearchResults(stations) {
  ...
  if (stations.length > 0) {
    ...
    headerItem.className =
      "list-group-item d-flex justify-content-between align-items-center bg-dark-subtle";
    ...
    function loadMore() {
      ...
      for (let i = 0; i < batchSize && index < stations.length; i++, index++) {
        ...
        listItem.className =
          "list-group-item list-group-item-action d-flex justify-content-between align-items-start px-1";
        ...
        if (station.country) {
          textContainer.appendChild(
            document.createTextNode(station.country + " ")
          );
        }
        ...
        if (station.votes) {
          ...
          spanVotes.className =
            "badge text-bg-dark border rounded-pill bi bi-hand-thumbs-up";
        ...
        if (station.clickcount) {
          ...
          spanClicks.className =
            "badge text-bg-dark border rounded-pill bi bi-mouse";
      ...
      if (index < stations.length) {
        ...
        loadMoreButton.className =
          "btn btn-primary mt-2 bi bi-chevron-double-down";
